...
async def game_next_round_handler(
    ...
    if query is None:
        return
    await query.answer()
    game_state = tg_runtime.game_state(context)
    ...
    if not topic_id or not mode_value:
        await query.edit_message_text(
            tg_runtime.tg("no_active_session_send_start", context=context, user=user)
        )
        return
    try:
        question = tg_runtime.start_training_session_with_ui_language(
            tg_runtime.service(context),
            user_id=user.id,
            topic_id=topic_id,
            lesson_id=lesson_id,
            mode=bot_module.TrainingMode(str(mode_value)),
            adaptive_per_word=True,
            ui_language=tg_runtime.telegram_ui_language(context, user),
        )
    except (bot_module.ApplicationError, ValueError) as error:
        await query.edit_message_text(str(error))
        return
    streak_days = tg_runtime.content_store(context).update_game_streak(
        user_id=user.id,
        played_at=datetime.now(UTC),
    )
    tg_runtime.set_user_data(
        context,
    ...
    await query.edit_message_text(
        tg_runtime.tg("game_round_started", context=context, user=user, streak_days=streak_days)
    )
...
async def game_repeat_handler(
    ...
    if query is None or user is None:
        return
    await query.answer()
    tg_runtime.pop_user_data(context, bot_module._GAME_STATE_KEY)
    bot_module._clear_medium_task_state(context)
    await query.edit_message_text(tg_runtime.tg("start_menu_title", context=context, user=user))
...
async def send_game_feedback(
    ...
    if outcome.result.is_correct:
        ...
        correct_answers += 1
        feedback = tg_runtime.tg("game_correct", context=context, user=getattr(message, "from_user", None))
    else:
        feedback = tg_runtime.tg("game_almost", context=context, user=getattr(message, "from_user", None))
    ...
    if outcome.summary is not None:
        ...
        active_session = tg_runtime.service(context).get_active_session(user_id=message.from_user.id)
...
async def finish_game_session(
    ...
    total_stars = tg_runtime.content_store(context).add_game_stars(user_id=user.id, stars=total_earned)
    streak_days = tg_runtime.content_store(context).update_game_streak(
        user_id=user.id,
        played_at=datetime.now(UTC),
    )
