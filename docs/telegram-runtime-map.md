...
Notifications are now slightly more self-contained too: [src/englishbot/telegram/notifications.py](/workspaces/EnglishBot/src/englishbot/telegram/notifications.py) owns its stable dismiss callback, timing windows, requeue callback wiring, and assignment-emoji helper locally instead of pulling those policy constants from `bot.py`. The `_PendingNotification` shape is still intentionally kept compatible with the bot facade for existing tests and wrapper callers.
